...
sudo python3 Utils/temperatureUpdate.py

# Initialize the final measurements CSV
echo "Benchmark,Program,PowerLimit,Package,Core,GPU,DRAM,Time,Temperature,Memory" > measurements.csv
...
for limit in -1 2 10 15 25; do
    sudo python3 Utils/raplCapUpdate.py "$limit" RAPL/main.c
    ...
    for program in "${programs_pyperformance[@]}"; do
        ...
        sudo tail -n +2 measurements.csv >> ../measurements.csv
        cd ..
    done
